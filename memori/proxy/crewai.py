...
class MemoriCrewAIAdapter:
    ...
    Captures agent conversations, tool calls, tool outputs, reasoning,
    and task results via CrewAI's callback system.
    """

    def __init__(
        self,
        memori_client: Memori | None = None,
        **kwargs: Any,
    ) -> None:
        from memori import Memori
        ...
        # CrewAI callbacks are synchronous — reject async clients to
        # prevent silent coroutine failures.
        client_name = self.client.__class__.__name__
        if "Async" in client_name:
            raise ValueError(
                "MemoriCrewAIAdapter only supports synchronous clients, "
                f"but received {client_name}."
            )
    ...
    def setup_crew(
        ...
        """Inject callbacks into a CrewAI Crew and its Agents to capture events."""
        ...
        def wrapped_task_callback(task_output: Any) -> None:
            try:
                ...
                raw_output = _extract(
                    task_output, "raw", str(task_output)
                )
                ...
                logger.error(
                    "MemoriCrewAIAdapter task_callback error: %s", e
                )
        ...
        if hasattr(crew, "agents"):
            for agent in crew.agents:
                original_step_callback = getattr(
                    agent, "step_callback", None
                )
                agent_role = getattr(agent, "role", "unknown_agent")

                def create_wrapped_step_callback(
                    role: str, orig_cb: Any
                ) -> Any:
                    def wrapped_step_callback(step_output: Any) -> None:
                        try:
                            ...
                            logger.error(
                                "MemoriCrewAIAdapter step_callback"
                                " error: %s",
                                e,
                            )
                        if orig_cb:
                ...
                agent.step_callback = create_wrapped_step_callback(
                    agent_role, original_step_callback
                )
