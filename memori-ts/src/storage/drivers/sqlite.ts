...
class Conversation {
  ...
  public async create(
    ...
    const existing = await this.conn.execute<{
      ...
      `SELECT c.id, c.project_id, COALESCE(MAX(m.date_created), c.date_created) as last_activity
         FROM memori_conversation c LEFT JOIN memori_conversation_message m ON m.conversation_id = c.id
        WHERE c.session_id = ? GROUP BY c.id, c.project_id, c.date_created`,
      [sessionId]
    );
    ...
    if (existing.length > 0) {
      ...
      if (result.length > 0 && result[0].minutes_since_activity <= timeoutMinutes) {
        if (projectId && existingRow.project_id !== projectId) {
    ...
    await this.conn.execute(
      `INSERT INTO memori_conversation(uuid, session_id, project_id) VALUES (?, ?, ?) ON CONFLICT DO NOTHING`,
...
class EntityFact {
  ...
  public async getFactsByIds(factIds: (string | number)[]): Promise<CandidateFactRow[]> {
    ...
    const summaryRows = await this.conn.execute<{
      ...
      `SELECT m.fact_id, c.summary AS content, COALESCE(c.date_updated, c.date_created) AS date_created, c.project_id AS project_id, s.uuid AS session_id, c.id AS conversation_id
