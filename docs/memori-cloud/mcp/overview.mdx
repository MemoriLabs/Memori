...
<Properties>
  ...
  <Property name="X-Memori-Process-Id" type="string" required>
    Process, app, or workflow identifier (e.g. `my_agent`).
  </Property>
</Properties>

<Note>
  `session_id` is generated automatically by the server from `entity_id` + UTC hour. You do not need to provide it.
</Note>
...
`entity_id` and `process_id` are read from the MCP headers. Returns a structured memory payload with relevant context.
...
`entity_id` and `process_id` are read from the MCP headers. Returns a confirmation: `memory being created`.
