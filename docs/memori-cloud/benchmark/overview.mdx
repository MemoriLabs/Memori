...
Evaluated on the LoCoMo benchmark, Memori achieves 87% accuracy, outperforming existing memory systems while using only 721 tokens per query (2.8% of full context). This results in substantial cost reductions, including 82% fewer tokens than competing approaches and over 36× savings compared to full-context methods.
