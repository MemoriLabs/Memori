...
- Pass the connection pool (`mysql.createPool()`), not a single connection (`mysql.createConnection()`) — Memori requires a pool to safely manage concurrent reads, writes, and background augmentation.
...
- Call `await pool.end()` after `mem.config.storage.close()` to drain and release all connections.
