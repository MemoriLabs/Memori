...
npm install --save-dev @types/pg @types/node typescript tsx
```
...
- Pass a `pg.Pool` instance, not a `pg.Client` — pooling is required for CockroachDB's connection model.
- Set `COCKROACHDB_CONNECTION_STRING` in your `.env` file.
- Call `await pool.end()` after `mem.config.storage.close()` to release the connection pool.
