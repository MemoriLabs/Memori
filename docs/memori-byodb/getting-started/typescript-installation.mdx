...
Memori supports SQLite, PostgreSQL, and MySQL/MariaDB through raw drivers and TypeORM. Install the package for your preferred setup:
...
| TypeORM       | `() => dataSource` (initialized `DataSource`)              | SQLite, PostgreSQL, MySQL, MariaDB |
...
## Using Drizzle, Sequelize, MikroORM, or Another ORM?
...
Your ORM handles your application's queries; Memori handles its own memory tables — same underlying pool, fully independent.
...
You own the lifecycle of whatever you passed to the factory — Memori never closes your pool or DataSource. Clean up your own resources after `close()`:
